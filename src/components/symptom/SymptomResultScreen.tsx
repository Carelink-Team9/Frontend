...
export default function SymptomResultScreen() {
  ...
  const result: DepartmentRecommendResponse | null = state?.result ?? null;
  ...
  const main = result ?? {
    mainDepartment: '분석 실패',
    translatedMainDepartment: '',
    mainConfidence: 0,
    reason: '증상 분석에 실패했습니다. 다시 시도해주세요.',
    translatedReason: '',
    doctorSummary: '',
    alternatives: [],
  };
  ...
  return (
          ...
          <button
            ...
            onClick={() =>
              ...
              navigate('/nearby-hospitals', {
                state: {
                  symptoms,
                  doctorSummary: main.doctorSummary,
                  mainDepartment: main.mainDepartment,
                },
              })
            }
    ...
    >
      <div className="flex flex-1 flex-col overflow-y-auto px-[20px] py-[24px] gap-[28px]">
          ...
          <div className="flex flex-col items-start gap-[12px] w-full rounded-[10px] bg-[rgba(41,109,255,0.1)] px-[24px] py-[24px]">
            <div className="flex w-full items-start justify-between">
              <div className="flex flex-col items-start gap-[12px]">
                <p className="text-left text-[24px] font-bold leading-[1.3] tracking-[-1.2px] text-[#111827] break-keep">
                  {main.mainDepartment}
                </p>
                {main.translatedMainDepartment && (
        ...
        {/* 대안 진료과 */}
        {main.alternatives && main.alternatives.length > 0 && (
          <div className="flex flex-col items-start gap-[12px] w-full">
            <p className="text-left text-[18px] font-medium leading-[1.3] tracking-[-0.9px] text-[#111827] break-keep">대안 진료과</p>
            <div className="flex flex-col gap-[12px] w-full">
              {main.alternatives.map((alt) => (
                <div
                  key={alt.departmentName}
                  className="flex flex-col items-start gap-[12px] w-full rounded-[10px] border border-[#d1d5db] bg-white px-[24px] py-[24px]"
                >
                  <div className="flex w-full items-start justify-between">
                    <div className="flex flex-col gap-[4px]">
                      <p className="text-left text-[18px] font-bold leading-[1.3] tracking-[-0.9px] text-[#111827] break-keep">
                        {alt.departmentName}
                      </p>
                      {alt.translatedDepartmentName && (
                        <p className="text-[13px] font-medium tracking-[-0.7px] text-[#6b7280]">
                          {alt.translatedDepartmentName}
                        </p>
                      )}
                    </div>
                    <div className="flex h-[35px] items-center rounded-[20px] bg-[#f3f4f6] px-[15px]">
                      <span className="text-[14px] font-medium tracking-[-0.7px] text-[#111827]">{alt.confidence}%</span>
                    </div>
                  </div>
                </div>
              ))}
