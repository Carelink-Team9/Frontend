...
export default function CommunityPage() {
  ...
  return (
    <MobileContainer
      hasBottomNav
      header={<AppHeader title={t('community.title')} showBack={false} />}
      bottomFixedElement={
    ...
    >
      ...
      <div className="flex flex-col items-center gap-[16px] px-[32px] py-[16px]">
        {loading ? (
          ...
          <StatusMessage message={t('community.empty')} />
        ) : (
          filteredPosts.map((post) => (
            <button key={post.postId} type="button" onClick={() => navigate(`/community/${post.postId}`)} className="w-full rounded-[10px] bg-[#f9f9fb] p-[20px] text-left shadow-[0px_4px_10px_0px_#d1d5db]">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-[10px]">
                  <div className="flex h-[40px] w-[40px] shrink-0 items-center justify-center rounded-full border border-[#d1d5db] bg-white shadow-[0px_1px_2px_0px_rgba(0,0,0,0.25)]">
                    <span className="text-[20px] font-bold tracking-[-1px] text-[#111827]">{(post.userName ?? String(post.userId)).slice(0, 1).toUpperCase()}</span>
                  </div>
                  <div className="flex flex-col gap-[2px]">
                    ...
                    <span className="text-[14px] font-medium tracking-[-0.7px] text-[#6b7280]">{formatTimeAgo(post.createdAt)}</span>
                  </div>
                </div>
                <span className="shrink-0 rounded-[15px] bg-[#6b7280] px-[10px] py-[3px] text-[14px] font-medium tracking-[-0.7px] text-white">{getCommunityCategoryLabel(post.category)}</span>
              </div>

              <div className="mt-[20px] flex flex-col gap-[10px]">
                <p className="line-clamp-1 text-[20px] font-bold tracking-[-1px] text-[#111827]">{post.title}</p>
                <p className="line-clamp-2 text-[16px] font-medium leading-[1.5] tracking-[-0.8px] text-[#6b7280]">{post.content}</p>
                {post.tag ? (
