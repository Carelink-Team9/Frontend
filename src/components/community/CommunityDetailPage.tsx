...
export default function CommunityDetailPage() {
  ...
  }, [currentLanguage, postIdNum, resetCommentText]);
  ...
  const handleAddComment = async () => {
    ...
    try {
      const nextComment = await addComment(postIdNum, {
  ...
  return (
    ...
    >
      {loading ? (
        <StatusMessage message="불러오는 중..." className="py-[60px]" />
      ) : !post ? (
        <StatusMessage message="게시글을 찾을 수 없습니다." className="py-[60px]" />
      ) : (
        <>
          <div className="bg-white px-[32px] pb-[20px] pt-[24px] text-left">
            ...
            <p className="mb-[24px] break-keep text-left text-[22px] font-bold leading-[1.4] tracking-[-1px] text-[#111827]">
              {post.title}
            </p>
            <p className="whitespace-pre-wrap text-left text-[15px] font-medium leading-[1.8] tracking-[-0.7px] text-[#4b5563]">
              {post.content}
            </p>
          ...
          <div className="bg-white px-[32px] pb-[20px] pt-[20px] text-left">
            ...
            <div className="flex flex-col gap-[24px]">
              {comments.length === 0 ? (
                <StatusMessage message="아직 댓글이 없습니다." className="py-[20px]" />
              ) : (
                comments.map((comment) => (
                  ...
                  >
                        ...
                        <div className="flex flex-col justify-center">
                          <span className="flex items-center gap-[6px] text-[15px] font-medium leading-[1.2] tracking-[-0.8px] text-[#111]">
                            ...
                            {comment.userLanguage ? (
                              <span className="text-[14px] leading-none">{getCommunityFlag(comment.userLanguage)}</span>
                            ) : null}
                          </span>
                          <span className="mt-[2px] text-[13px] font-medium leading-[1.2] tracking-[-0.7px] text-[#8c929e]">
                    ...
                    <p className="mt-[20px] whitespace-pre-wrap text-left text-[14px] font-medium leading-[1.7] tracking-[-0.7px] text-[#6b7280]">
                      {comment.content}
                    </p>
